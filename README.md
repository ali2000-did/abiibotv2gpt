...
## 🖥 نصب روی ویندوز — فقط با دابل‌کلیک

1. یک بار [Python 3.10+](https://www.python.org/downloads/) را نصب کنید —
   حتماً گزینه **"Add python.exe to PATH"** را تیک بزنید.
2. زیپ پروژه را دانلود و Extract کنید (راست‌کلیک ← Extract All).
3. روی **`activate.bat`** دابل‌کلیک کنید — همین!

نصب، تست سلامت، شروع ربات و **باز شدن داشبورد در مرورگر** همه خودکار است.
آخر کار می‌پرسد «اجرا خودکار هنگام روشن شدن ویندوز؟» — با y می‌پذیرید تا
ربات با هر بار روشن شدن سیستم، بی‌صدا همراه داشبورد بالا بیاید
(Task Scheduler). داشبورد فقط روی همین سیستم (127.0.0.1) باز می‌شود؛
پس پنجره فایروال ویندوز هم باز نمی‌شود و شماره‌ها بیرون نمی‌روند.

سایر دستورها (دابل‌کلیک روی activate.bat یا در CMD داخل پوشه پروژه):

```bat
activate.bat --status   :: وضعیت ربات و داشبورد
activate.bat --stop     :: توقف کامل
activate.bat --start    :: فقط شروع (بدون نصب مجدد)
```

نکته: ترب فقط از IP ایران جواب می‌دهد؛ سیستم ویندوزی شما داخل ایران است
پس اسکن واقعی همین‌جا کار می‌کند (به‌جای سرور خارجی).
